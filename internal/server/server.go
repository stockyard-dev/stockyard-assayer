package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-assayer/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-assayer"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/revenue",s.handleList)
    s.mux.HandleFunc("POST /api/revenue",s.handleRecord)
    s.mux.HandleFunc("DELETE /api/revenue/{id}",s.handleDelete)
