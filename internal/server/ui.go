...
async function load(){var r=await fetch(A+'/subscriptions').then(function(r){return r.json()});items=r.subscriptions||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.customer_name||'').toLowerCase().includes(q)||(i.plan||'').toLowerCase().includes(q)||(i.mrr||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.customer_name)+'</div>';
if(i.plan)h+='<div class="item-sub">'+esc(i.plan)+'</div>';
...
if(i.mrr)h+='<span>'+esc(i.mrr)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.start_date)h+='<span>'+esc(i.start_date)+'</span>';
if(i.renewal_date)h+='<span>'+esc(i.renewal_date)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
if(i.notes)h+='<div class="item-notes">'+esc((i.notes||'').substring(0,200))+'</div>';
h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/subscriptions/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{customer_name:"",plan:"",mrr:"",status:"",start_date:"",renewal_date:"",notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+'</h2>';
h+='<div class="fr"><label>Customer Name *</label><input id="f-customer-name" value="'+esc(i.customer_name)+'"></div>';
h+='<div class="fr"><label>Plan</label><input id="f-plan" value="'+esc(i.plan)+'"></div>';
h+='<div class="fr"><label>M R R</label><input id="f-mrr" value="'+esc(i.mrr)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Start Date</label><input id="f-start-date" value="'+esc(i.start_date)+'"></div>';
h+='<div class="fr"><label>Renewal Date</label><input id="f-renewal-date" value="'+esc(i.renewal_date)+'"></div>';
h+='<div class="fr"><label>Notes</label><textarea id="f-notes" rows="3">'+esc(i.notes)+'</textarea></div>';
...
async function submit(){var body={customer_name:document.getElementById('f-customer-name').value.trim(),plan:document.getElementById('f-plan').value.trim(),mrr:document.getElementById('f-mrr').value.trim(),status:document.getElementById('f-status').value.trim(),start_date:document.getElementById('f-start-date').value.trim(),renewal_date:document.getElementById('f-renewal-date').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/subscriptions/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/subscriptions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
